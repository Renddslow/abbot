...
const filterResults = <T>(search: string, arr: Array<T>): Array<[Keys | null, T]> => {
  ...
  const listWithPaths: Array<[Keys | null, T]> = filteredList.map((d) => [
    objectSearch(d, search),
    d,
  ]);

  return listWithPaths.filter(([f]) =>
    f ? !f.every((d) => d.path.includes('avatar') || d.path.includes('__typename')) : true,
  );
};
